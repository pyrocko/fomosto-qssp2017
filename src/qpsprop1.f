      subroutine qpsprop1(ypsv,lyup,lylw)
      use qpalloc
      implicit none
c
...
c     work space
c
      integer*4 i,j,j0,istp,ly,lyswap,ldeg,key
      real*8 y4max
...
c
      if(lys.ge.lyob)then
        if(lyup.lt.lyob)then
          ...
          if(comsys.and.lyup.eq.lyob)call cmemcpy(yup,y0,18)
        endif
      endif
c
c===============================================================================
...
c
          ...
          if(key.eq.0)then
            print *,' Warning in qpsprop1: anormal exit from cdsvd500!'
            return
          endif
          if(lyr.le.lys)then
            do istp=1,2
